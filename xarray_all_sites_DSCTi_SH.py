...
# STOP: q2 is not the specific surface humidity, it seems!

ds_HRCM = xr.open_dataset('/home/haslebacher/chaldene/Astroclimate_Project/sites/MaunaKea/Data/HRCM/q/Hawaii_Regional_Climate_Model_Simulations_Hourly_3km_domain_Present_1990_2009_SH.nc')

ds_HRCM_sel = ds_HRCM.sel(lon=lon, lat=lat, method='nearest')

ds_HRCM_yearly = ds_HRCM_sel['q2'].groupby('time.year').mean()
ds_HRCM_monthly = ds_HRCM_sel['q2'].groupby('time.month').mean()
...
fig.savefig('./Model_evaluation/SH/MaunaKea_qv_mixing_ratio_Timeseries_seasonal_cycle.pdf')

# quickly plot ds_hourly_sel and ds_HRCM

# idea: save HRCM dataset where ERA5 is saved and run code!!
...
# ylabel = r'Specific Humidity [$kg/m^2$]'
...
#     elif idx == 0 or idx == 7: # Mauna Kea, SPM
#         list_of_model_clim_params = ['hus']
