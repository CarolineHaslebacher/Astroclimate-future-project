...
Ensemble = True
masterfigure = True
save = False # change back to True
model = True
model_color=False
...
for idx in range(0, max_idx+1):
    ...
    if idx == 4: # La Palma
        ls_pr_levels_ERA5 = [850]
    if idx == 5: # siding spring
        list_of_insitu_vars = ['siding spring Relative Humidity']
        ls_pr_levels_ERA5 = [950, 1000]
    if idx == 6: # sutherland
        ls_pr_levels_ERA5 = [850, 875]
    if idx == 7: # SPM
        ls_pr_levels_ERA5 = [750, 850]
    ...
    if save:
        ...
        if masterfigure: # only save the masterfigure in the end
            if idx == max_idx:
                print('saving')
