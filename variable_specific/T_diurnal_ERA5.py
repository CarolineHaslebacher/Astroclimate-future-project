...
for idx in range(0, max_idx+1):
    ...
    if idx == 2: # La Silla
        list_of_insitu_vars = ['La_Silla T 2m', 'La_Silla T 30m', 'La_Silla T ground']
    if idx == 3: # Cerro Tololo
        list_of_insitu_vars = ['Cerro Tololo Temperature']
    # if idx == 4: # La Palma
    #     ls_pr_levels_ERA5 = [800] # revision: why so many?? [700, 750, 775, 800]
    ...
    # # save as .pdf
    # do not save (and therefore replace) timeseries/seasonal cycle
    # if masterfigure:
    #     if idx == max_idx:
    #         fig.savefig('./Model_evaluation/' + variable + '/'  + variable +'_overview_Ensemble_DSC_T.pdf')
