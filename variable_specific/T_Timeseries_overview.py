...
for idx in range(0, max_idx+1):
    ...
    if idx == 2: # La Silla
        list_of_insitu_vars = ['La_Silla T 2m', 'La_Silla T 30m'] # , 'La_Silla T ground' # doesn't add
    if idx == 3: # Cerro Tololo
        list_of_insitu_vars = ['Cerro Tololo Temperature']
        ls_pr_levels_ERA5 = [775, 800]
    if idx == 4: # La Palma
        ls_pr_levels_ERA5 = [700, 750]
    if idx == 5: # siding spring
        list_of_insitu_vars = ['siding spring Temperature']
        ls_pr_levels_ERA5 = [875, 900]
        ls_pr_levels_clim_model = [850, 925]
    if idx == 6: # SUtherland
        ls_pr_levels_ERA5 = [825, 875]
    if idx == 7: # SPM
        ls_pr_levels_ERA5 = [700, 750]
