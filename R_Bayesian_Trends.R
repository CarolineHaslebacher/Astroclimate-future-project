...
cvars <- c( 'T', 'total_cloud_cover', 'SH', 'RH', 'TCW', 'seeing_osborn', 'wind_speed_seeing')
...
for (variable in cvars){
  ...
  for (forc in cforcing){
    ...
    pdf(width=10, height=10, pointsize=10, file=paste(base_path, variable, '_', forc, "_Linear_fit_monthly_timeseries_map2stan.pdf" , sep=''))
