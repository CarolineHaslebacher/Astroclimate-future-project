...
#for idx in range(6, 7):
    ...
    list_of_insitu_vars = [site_name_folder + ' Pressure']
    ...
    if idx == 0: # Mauna Kea
        list_of_insitu_vars = ['Mauna Kea Pressure']
        ds_hourly = ds_hourly.rename_vars({'P': 'Mauna Kea Pressure'})

        # ls_pr_levels_ERA5 = [600] # change back after layout is fine
    if idx == 1: # Paranal
        list_of_insitu_vars = ['Paranal Pressure']
        # ds_hourly = ds_hourly.rename_vars({'P': 'Paranal Pressure'})
     
    if idx == 3: # Cerro Tololo
        list_of_insitu_vars = ['Cerro Tololo Pressure']
        ds_hourly = ds_hourly.rename_vars({'Cerro Tololo Pressure (hPa)': list_of_insitu_vars[0]})


    if idx == 5: # siding spring
        list_of_insitu_vars = ['siding spring Pressure']
    ...
    if model:
        d_model = {"HadGEM": {"folders": ['hist'],"taylor_folder": ['hist'],"single_lev_var": list_of_single_level_model_clim_params, "name": "HadGEM3-GC31-HM"},
               "EC-Earth": {"folders": ['hist'],"taylor_folder": ['hist'],"single_lev_var": list_of_single_level_model_clim_params, "name": "EC-Earth3P-HR"},
                "CNRM": {"folders": ['hist'], "taylor_folder": ['hist'] ,"single_lev_var": list_of_single_level_model_clim_params, "name": "CNRM-CM6-1-HR"},
                "MPI": {"folders": ['hist'],"taylor_folder": ['hist'] ,"single_lev_var": list_of_single_level_model_clim_params, "name": "MPI-ESM1-2-XR"},
                "CMCC": {"folders": ['hist'],"taylor_folder": ['hist'] ,"single_lev_var": list_of_single_level_model_clim_params, "name": "CMCC-CM2-VHR4"},
                "ECMWF": {"folders": ['hist'],"taylor_folder": ['hist'] ,"single_lev_var": list_of_single_level_model_clim_params, "name": "ECMWF-IFS-HR"}}
        
    else:
        d_model = None

    if Ensemble:
        d_Ensemble = {"folders": ['hist'], "single_lev_var": list_of_single_level_model_clim_params}
    else:
        d_Ensemble = None
