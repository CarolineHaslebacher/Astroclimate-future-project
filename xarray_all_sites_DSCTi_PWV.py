...
ds_HRCM = xr.open_dataset('/home/haslebacher/chaldene/Astroclimate_Project/sites/MaunaKea/Data/HRCM/tcw/Hawaii_Regional_Climate_Model_Simulations_Hourly_3km_domain_Present_1990_2009_tcw.nc')
ds_HRCM_sel = ds_HRCM.sel(lon=lon, lat=lat, method='nearest')

# liquid water path = lwp
ds_HRCM_yearly = ds_HRCM_sel['lwp'].groupby('time.year').mean() * 100 # unit?
ds_HRCM_monthly = ds_HRCM_sel['lwp'].groupby('time.month').mean() * 100
...
#         list_of_model_clim_params = ['q_integrated']
#         ls_pr_levels_clim_model , "single_lev_var": list_of_single_level_model_clim_params
...
#%% Mauna Kea (check why there are not only 4UT to 17UT values)
# hour = 3
# ds_hour = d_obs['ds_sel']['MaunaKea PWV'].where(d_obs['ds_sel']['MaunaKea PWV']['time.hour'] == hour, drop = True)
# ds_nonan = ds_hour.where(xr.ufuncs.isnan(ds_hour) != True, drop = True)
# ds_nonan.size
...
#%%
# climate models: SH integral (INCLUDING CHECK FOR PRIMAVERA UNTIL MODEL PRESSURE LEVEL)
