...
##########################################
# select above or use for loop
cvars <- c( 'seeing_osborn', 'wind_speed_seeing')

for (variable in cvars){

  
    
  if (variable == 'seeing_osborn') {
    print('seeing (osborn)')
    ...
    cforcing <- c('hist', 'present') # 'future', 'hist', 
    ...
    ylabel <- 'Astronomical seeing [arcesc]'
    cpressure <- c('_single_level')
    unit <- 'arcsec'
    offset <- 0.6
    csites <- c("Mauna_Kea")
    # csites <- c("Mauna_Kea", "Cerro_Paranal")
    cforcing <- c('hist') # 'future', 'hist', 
  ...
  for (forc in cforcing){
    ...
    pdf(width=10, height=10, pointsize=10, file=paste(base_path, variable, '_', forc, "_Mauna_Kea_fix.pdf" , sep=''))
    ...
    for (idx in seq_along(csites)) {
        ...
        try(
          ...
          m1 <- map2stan(
            alist(
              ...
              b ~ dnorm( 0 , 3) ,
              Amp ~ dnorm(4, 6), # makes only sense if positive...! 
              x ~ dnorm(0 , 2),
              sigma ~ dunif( 0 , 10 )
            ) ,
            data=d.trim, chains=2 , iter=4000 , warmup=1000  ) #,
      ...
      sub_dir <- paste(variable,'/', csites[idx], '/PRIMAVERA/', forc, '/' , sep='')
