...
for (variable in cvars) {
  ...
  # width=8, height=18
  # make quadratic!
  pdf(width=10, height=10, pointsize=10, file=paste(base_path, variable, "_Siding_Spring_fix.pdf" , sep=''))
      ...
      try(
        ...
        m1 <- map2stan(
          alist(
            ...
            b ~ dnorm( 0 , 3) ,
            Amp ~ dnorm(0, 6),
            x ~ dnorm(0 , 2), # changed to a smaller number (from 6 to 2), because it seems that map2stan oscillates between 
