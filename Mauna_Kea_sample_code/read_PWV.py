...
base_url = "https://www.cadc-ccda.hia-iha.nrc-cnrc.gc.ca/data/pub/JCMT/"
...
# days_2005_to_2019 = np.arange('2009-02', '2020-01', dtype='datetime64[D]')
days_2005_to_2019 = np.arange('2018-08', '2020-01', dtype='datetime64[D]')

tables = []
# # initialize list with zeros (to omit .append for performance)
# n = len(days_2005_to_2019) # This is for sure enough
# tables = [0] * n
...
labels = ['time','col2', 'col3', 'col4', 'col5', 'col6', 'col7', 'col8','col9','PWV', 'col11', 
    'col12', 'col13', 'col14', 'col15']
# time is in UTC

# implement 'try' 'except' for errors where no data is available under the composed URL
# for loop i year (= 365 days)
with open('MaunaKea_PWV_missing_dates_2018.txt', 'w') as f:
    print('missing_dates_MK_PWV', file=f)
    k = 0 # index for list (there are missing values, so i is not a good choice)
    for i in range(0, len(days_2005_to_2019)):
        try:

            t = days_2005_to_2019[i].astype(dt.datetime)
            t = t.strftime('%Y%m%d')
            #print(t)
            #print(base_url + '{0}.wvm'.format(t))
            with open(download_file(base_url + '{0}.wvm'.format(t), cache=True), 'r') as file:
                raw_table = file.read()
            ...
            table = table[((pd.DatetimeIndex(table['col1']).minute == 57) & (pd.DatetimeIndex(table['col1']).second >= 30)) 
                | (pd.DatetimeIndex(table['col1']).minute > 57) | (pd.DatetimeIndex(table['col1']).minute < 2)
                | ((pd.DatetimeIndex(table['col1']).minute == 2) & (pd.DatetimeIndex(table['col1']).second <= 30))]
...
df.to_csv('./sites/MaunaKea/Data/in-situ/PWV/hourly_PWV_MK_JCMT_instantaneously_2018to2019.csv', header = True, index = True)
